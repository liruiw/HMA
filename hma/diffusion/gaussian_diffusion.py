...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        def process_xstart(x):
            if denoised_fn is not None:
                x = denoised_fn(x)
            if clip_denoised:
                range = 10  # 1 for images but 3 for vae latents?
                return x.clamp(-range, range)
            return x
    ...
    def p_sample(
        self, model, x, t, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None, temperature=1.0
    ):
        """
